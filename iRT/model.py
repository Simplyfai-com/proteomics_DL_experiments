...
class Block(nn.Module):
    ...
    #    x = x + y*self.attn_gate
    #    x = x + self.mlp(self.norm2(x))*self.ff_gate
    #    return x
...
class Transformer(nn.Module):
    ...
    def forward_enc(self, input_sequence):
        ...
        cls_tokens = x[:, 0]
        pool_feats = self.pooling(x[:, 1:].permute(0,2,1)).squeeze(-1)

        feat_vec = torch.cat((cls_tokens, pool_feats), dim=-1)
        return feat_vec
